...
//    }
    ...
    public void send (byte[] data) {
		...
		// have counter for intermediate array
		// append bytes seen to intermediate array at position counter. increment counter
		// if intermediate is full, then create frame of intermediate and transmit that frame. Then set intermediate frame to empty & counter to 0.
		
		int counter = 0;
		List<Byte> intermediateFrame = new ArrayList<>();
		for (int i = 0; i<data.length; i++) {
			intermediateFrame.add(data[i]);
			counter++;
			if (counter == 8) {
				byte [] toTransmit = createFrame(getByteArray(intermediateFrame));
				for (int j = 0; j<toTransmit.length; j++) {
					transmit(toTransmit[j]);
				}
				intermediateFrame = new ArrayList<>();
				counter = 0;
			}
		}
		
		//handle remaining data bytes
		//this chunk will be hit for all data <8 bytes
		if (!intermediateFrame.isEmpty()) {
			byte [] toTransmit = createFrame(getByteArray(intermediateFrame));
			for (int j = 0; j<toTransmit.length; j++) {
				transmit(toTransmit[j]);
			}
	...
	private byte[] getByteArray(List<Byte> intermediateFrame) {
		byte[] toRet = new byte[intermediateFrame.size()];
		for (int i = 0; i<toRet.length; i++) {
			toRet[i] = intermediateFrame.get(i);
		}
		return toRet;
	}
    // =========================================================================
